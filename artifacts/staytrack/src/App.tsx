...
function SelectField({ label, name, options, form, setForm }: { label: string; name: string; options: { value: string; label: string }[]; form: Record<string,string>; setForm: (updater: (prev: Record<string,string>) => Record<string,string>) => void }) {
  return <label className="block"><span className="mb-1.5 block text-xs font-bold text-[#48605c]">{label}</span><select value={form[name] || ''} onChange={event => setForm(prev => ({ ...prev, [name]: event.target.value }))} className="w-full rounded-xl border border-[#d9d2c4] bg-[#f4f0e8] px-3 py-2.5 text-sm outline-none focus:border-[#c8664d]" data-testid={`select-${name}`}><option value="">Select {label.toLowerCase()}</option>{options.map(option => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label>;
}
...
function BudgetActualPage() {
  ...
  const rows = (budgets || []).map((budget) => {
    ...
    const actual = a?.monthly.find((item) => item.month === month)?.expenses || 0;
